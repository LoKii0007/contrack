...
const EditOrder = () => {
  ...
  useEffect(() => {
    if (order) {
      ...
      // Set payment history if it exists
  ...
  const handlePaymentChange = (index, field, value) => {
    setPaymentHistory((prev) => {
      ...
      updated.forEach((payment, i) => {
        cumulativePaid += parseFloat(payment.creditAmount) || 0;
        updated[i].remainingAmount = Math.max(0, formData.total - cumulativePaid);
      });
  ...
  if (isLoadingOrder || isLoadingProducts || isLoadingUsers) return <Loader />;
        ...
        <form onSubmit={handleSubmit} className="mx-auto max-w-5xl space-y-8">
                ...
                {isAddingNewUser ? (
                  ...
                  <Popover open={openUserCombobox} onOpenChange={setOpenUserCombobox}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {formData.customer
                          ? users.find((user) => user._id === formData.customer)?.name
                          : "Select customer..."}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                              ...
                              <div className="space-y-1">
                                <Label htmlFor={`quantity-${item.product._id}`} className="text-xs">
                                  Quantity
                                </Label>
                                <Input
                              ...
                              {/* Price */}
                              <div className="space-y-1">
                                <Label htmlFor={`price-${item.product._id}`} className="text-xs">
                                  Price ($)
                                </Label>
                                <Input
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            ...
            <p className="mb-4 text-sm text-muted-foreground">
              Optionally add payment history while editing the order
            </p>
              ...
              <div className="space-y-4">
                {paymentHistory.map((payment, index) => {
                  const totalPaidBefore = paymentHistory
                    .slice(0, index)
                    .reduce((sum, p) => sum + (parseFloat(p.creditAmount) || 0), 0);
                    ...
                    >
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              handlePaymentChange(index, "paymentDate", dateValue);
                            }}
                            className="h-9"
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              handlePaymentChange(index, "creditAmount", amount);
                            }}
                            className="h-9"
                            placeholder="0.00"
                          />
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            <SelectTrigger id={`paymentMethod-${index}`} className="h-9">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="cash">Cash</SelectItem>
                        ...
                        {/* Receiver */}
                        <div className="space-y-2">
                          <Label htmlFor={`receiver-${index}`}>Receiver (Optional)</Label>
                          <Input
                            type="text"
                            id={`receiver-${index}`}
                            value={payment.receiver || ""}
                            onChange={(e) =>
                              handlePaymentChange(index, "receiver", e.target.value)
                            }
                            className="h-9"
